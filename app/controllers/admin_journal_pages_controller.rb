class AdminJournalPagesController < ApplicationController
  ...
  def page_params
    par = params.require(:page).permit(:id, :journal_id, :head, :content_text, :content_line, :page_type, tag_tokens:[:id, :name], images_attributes:[:remote_image_url, :remote_url, :id], blocks_attributes: [:id, :content, :remote_image_url, :remote_url ])
