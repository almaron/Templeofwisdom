class ForumTopicsController < ApplicationController
  ...
  def index
    ...
    @all_topics = (current_user && current_user.can_view_hidden?) ? @forum.topics : @forum.topics.shown
      ...
      if @topic.is_available?(current_user) && topic_in_place
        ...
        format.json {
          if current_user
            ...
            @chars = @topic.forum.technical > 0 ? current_user.chars.eager_load(:avatars).where('status_id IN (3,4,5)') : current_user.chars.eager_load(:avatars).where(status_id: 5)
          end
        }
