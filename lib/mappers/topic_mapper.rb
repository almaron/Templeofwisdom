module Mappers
  class TopicMapper
    ...
    def map
      ...
      db.query("SELECT topic_id as id, forum_id, topic_title as head, topic_first_poster_name as poster FROM hor_topics WHERE id >= #{start_id}").each_with_index do |row, index|
        map_topic row, index
      end
      dump_chars
    end
